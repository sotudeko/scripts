...
# curl -u admin:admin123 -X POST -H "Content-Type: application/json" http://localhost:8070/api/v2/components/details?pkg:${format}/${groupId}/{${artifactId}}@${version}?type=${type}

curl -u admin:admin123 -X POST -H "Content-Type: application/json" -d "{\"components\":[{\"packageUrl\":\"pkg:${format}/${groupId}/{${artifactId}}@${version}?type=${type}\"}]}" 'http://localhost:8070/api/v2/components/details'
